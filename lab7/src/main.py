...
def check_slopeintercept(title, filename, m_gold, b_gold):
    ...
    if (similar(line.m, m_gold, .1)):
        print("m: " + str(line.m) + " (" + bcolors.GREEN +
              "SUCCESS" + bcolors.NORMAL + ")")
    else:
        print("m: " + str(line.m) + " (" + bcolors.RED + "FAIL" +
              bcolors.NORMAL + ") expected " + str(m_gold))
        success = False

    if (similar(line.b, b_gold, 3)):
        print("b: " + str(line.b) + " (" + bcolors.GREEN +
              "SUCCESS" + bcolors.NORMAL + ")")
    else:
        print("b: " + str(line.b) + " (" + bcolors.RED + "FAIL" +
              bcolors.NORMAL + ") expected " + str(b_gold))
        success = False
    print
    return success
...
def check_circles(title, filename, c_gold):
    ...
    if (circles == c_gold):
        print("detected: " + str(circles) +
              " (" + bcolors.GREEN + "SUCCESS" + bcolors.NORMAL + ")")
    else:
        print("detected: " + str(circles) + " (" + bcolors.RED +
              "FAIL" + bcolors.NORMAL + ") expected " + str(c_gold))
        success = False
    print
    return success
...
all_passed = check_slopeintercept(
    "Line 1", "../test1.ppm", -1.0, 300) and all_passed
all_passed = check_slopeintercept(
    "Line 2", "../test2.ppm", .85, 26) and all_passed
all_passed = check_slopeintercept(
    "Line 3", "../test3.ppm", .215, 97) and all_passed
all_passed = check_slopeintercept(
    "Line 4", "../test4.ppm", 2.0, -100) and all_passed
all_passed = check_slopeintercept(
    "Line 5", "../test5.ppm", 5.0, 0) and all_passed
