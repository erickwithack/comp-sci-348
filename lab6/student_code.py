        ...
        for y in range(MAP_HEIGHT):
            for x in range(MAP_WIDTH):
                if map[y][x] != common.constants.RIVAL and map[y][x] != common.constants.CUSTOMER:
                    value_list = find_next_values(
                        map, values, delivery_fee, battery_drop_cost, dronerepair_cost, discount, y, x)
...
def find_next_values(map, values, delivery_fee, battery_cost, repair_cost, gamma, y, x):
    ...
    reward_off = find_reward(
        map, delivery_fee, battery_cost, repair_cost, False, y, x)
    reward_on = find_reward(
        map, delivery_fee, battery_cost, repair_cost, True, y, x)

    south_off = get_direction_value(
        reward_off, False, gamma, south_val, east_val, west_val)
    west_off = get_direction_value(
        reward_off, False, gamma, west_val, south_val, north_val)
    north_off = get_direction_value(
        reward_off, False, gamma, north_val, east_val, west_val)
    east_off = get_direction_value(
        reward_off, False, gamma, east_val, south_val, north_val)

    south_on = get_direction_value(
        reward_on, True, gamma, south_val, east_val, west_val)
    west_on = get_direction_value(
        reward_on, True, gamma, west_val, south_val, north_val)
    north_on = get_direction_value(
        reward_on, True, gamma, north_val, east_val, west_val)
    east_on = get_direction_value(
        reward_on, True, gamma, east_val, south_val, north_val)
...
def get_direction_value(reward, special_propulsion, gamma, main_dir, right_dir, left_dir):
    if special_propulsion:
        direction_value = 0.8 * get_partial_direction_value(reward, gamma, main_dir)\
            ...
            + 0.1 * \
            get_partial_direction_value(reward, gamma, left_dir)
    else:
