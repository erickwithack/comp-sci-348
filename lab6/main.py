...
def check_values(map, map_gold,  show):
    ...
    for y in range(6):
        ...
        for x in range(6):
            value = map[y][x]
            if withinprc(value, map_gold[y*6+x], .001):
                v += bcolors.GREEN
            else:
                result = False
                v += bcolors.RED
            v += "{:05.2f}\t".format(value)
        v += bcolors.NORMAL
        if (show):
            print(v)
    return result
...
def run_experiment(data, value_gold, p_gold, values_gold, delivery_fee, battery_drop_cost, dronerepair_cost, discount_per_cycle):
    ...
    value = student_code.drone_flight_planner(
        map, pp, vv, delivery_fee, battery_drop_cost, dronerepair_cost, discount_per_cycle)

    pp_gold = init_map()
    set_policies(pp_gold, p_gold)

    if (not check_policies(pp, pp_gold, True)):
    ...
    if (not check_values(vv, values_gold, True)):
    ...
    if withinprc(value, value_gold, .01):
        print("Delivery job value: "+bcolors.GREEN +
              "SUCCESS"+bcolors.NORMAL+")", value)
    else:
        print("Delivery job value: "+bcolors.RED+"FAIL" +
              bcolors.NORMAL, value, " - correct value ", value_gold)
        result = False
    return result
...
print("Map1:")
all_passed = run_experiment(data1, 47.06, p_gold1,
                            values_gold1, 100, 1, 100, .95) and all_passed
print
...
print("Map2:")
all_passed = run_experiment(
    data2, -20.683084, p_gold2, values_gold2, 50, 5, 200, .95) and all_passed
print
...
print("Map3:")
all_passed = run_experiment(data3, 171.97, p_gold3,
                            values_gold3, 200, .1, 500, .99) and all_passed
print
...
print("Map4:")
all_passed = run_experiment(data4, 2830.98, p_gold4,
                            values_gold4, 10000, 100, 500, .9) and all_passed
print
...
print("Map5")
all_passed = run_experiment(data5, 8643.62, p_gold5,
                            values_gold5, 10000, .1, 500, .99) and all_passed
print
...
print("Map6")
all_passed = run_experiment(
    data6, -16768.19, p_gold6, values_gold6, 10000, 5000, 500, .9) and all_passed
print
